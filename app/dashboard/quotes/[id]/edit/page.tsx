...
import type { Quote, SitePin } from "@/lib/types";
...
export default function EditQuotePage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/quotes/${id}`)
      ...
      .then((q: Quote) => {
        ...
        setCustomerPhone(formatPhoneInput(q.customer_phone ?? ""));
        setCustomerEmail(q.customer_email ?? "");
        setAddress(q.property_address ?? "");
        setCity("");
        setDate(q.date ?? "");
        setSalesRep(q.sales_rep ?? "");
        setWetDry(q.wet_dry ?? "");
        setLeadSource(q.lead_source ?? "");
        setHoursEstimate(q.hours_estimate ?? "");
        setNotes({
          pendingHoa:    q.pending_hoa    ?? false,
          cityPermit:    q.city_permit    ?? false,
          locate811:     q.locate_811     ?? false,
          mainLines:     q.main_lines     ?? false,
          powerDrop:     q.power_drop     ?? false,
          arboristOnsite: q.arborist_onsite ?? false,
        });
        setDescriptionOfWork(q.description_of_work ?? "");
        setSiteMapPins(q.site_map_pins ?? []);
        const equip = q.equipment ?? [];
        setEquipmentNeeded(equip.length > 0);
        setEquipment(equip);
        setTreeServicesCost(q.tree_services_cost ?? 0);
        setStumpRemovalCost(q.stump_removal_cost ?? 0);
        setDiscount(q.discount ?? 0);
        setCardFeeApplied(q.card_fee_applied ?? false);
        setCustomerSignature(q.customer_signature ?? null);
        setSignedAt(q.signed_at ?? null);
      })
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      >
        <ChevronLeft size={16} />
        <span className="group-hover:text-[#1A1A1A] transition-colors">Back to Quote</span>
      </Link>
      ...
      >
        ...
        <div>
          ...
          {customerSignature && !showSignaturePad ? (
            <div>
              <div
